...
import { Activity, Server, Users, AlertTriangle, Cpu, HardDrive, Globe, Bell, Zap, Clock } from 'lucide-react'
import PageTransition from '../components/PageTransition'

function LiveCounter({ label, value, unit, icon: Icon, color }: any) {
  const [v, setV] = useState(value)
  useEffect(() => {
    const t = setInterval(() => setV((p: number) => +(p + (Math.random() - 0.5) * 0.6).toFixed(1)), 1500)
  ...
  return (
    <motion.div
      ...
      className="bg-[#0d1117]/80 backdrop-blur-sm border border-white/[0.05] rounded-xl p-4 hover:border-white/[0.1] hover:shadow-[0_4px_20px_rgba(99,102,241,0.08)] transition-all duration-300 group"
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] text-slate-500 uppercase tracking-wider">{label}</span>
        <Icon size={14} className={`${color} group-hover:scale-110 transition-transform duration-300`} />
      </div>
      ...
      <div className="flex items-end gap-1.5" style={{ height }}>
        {data.map((h, i) => (
          <motion.div
            ...
            transition={{ delay: i * 0.03, duration: 0.6, ease: [0.22, 0.1, 0.2, 1] }}
            className="flex-1 rounded-t-sm hover:opacity-90 transition-all cursor-pointer relative group"
            style={{ backgroundColor: color, opacity: 0.4 + (h / max) * 0.6 }}
          >
            <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] text-white opacity-0 group-hover:opacity-100 whitespace-nowrap bg-black/80 px-1.5 py-0.5 rounded transition-opacity">
              {h}
            </div>
          </motion.div>
        ))}
        ...
        {segments.map(seg => {
          ...
          return (
            <motion.circle
              ...
              animate={{ strokeDashoffset: 25 + offset }}
    ...
    <PageTransition>
      ...
      <div className="pt-20 bg-slate-900/50 border-b border-white/[0.04]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-3">
          <p className="text-xs text-slate-500">
            技术展示 · Demo 页面 — 实时监控大屏：多图表类型、地理区域状态、事件时间线、实时指标。
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-6 space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-xl font-bold text-white">系统监控中心</h1>
            <p className="text-xs text-slate-500 mt-0.5">实时数据 · 自动刷新 · v3.2.1</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5 text-[10px] text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-full border border-emerald-400/20">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.4)] animate-pulse" />
              在线
            </span>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="bg-[#0d1117]/80 backdrop-blur-sm border border-white/[0.05] rounded-2xl p-5 hover:border-white/[0.08] transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-white">请求量趋势</h3>
              <div className="flex gap-1 bg-white/[0.03] rounded-lg p-0.5">
                {['1h', '6h', '12h', '24h'].map(t => (
                  <button key={t} className="px-2.5 py-1 text-[10px] rounded-md text-slate-500 hover:text-white hover:bg-white/[0.06] transition-all">
                    {t}
                  </button>
                ))}
              </div>
            ...
            <BarChart
              data={[42, 58, 45, 72, 55, 68, 78, 62, 85, 70, 90, 82]}
              color="#3b82f6"
              height={160}
          ...
          <div className="bg-[#0d1117]/80 backdrop-blur-sm border border-white/[0.05] rounded-2xl p-5 hover:border-white/[0.08] transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full">↑ 12.5%</span>
            </div>
            <BarChart
              data={[120, 135, 128, 145, 138, 155, 142, 160, 148, 172, 155, 168]}
              color="#8b5cf6"
              height={160}
              labels={['00', '02', '04', '06', '08', '10', '12', '14', '16', '18', '20', '22']}
            />
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="bg-[#0d1117]/80 backdrop-blur-sm border border-white/[0.05] rounded-2xl p-5 hover:border-white/[0.08] transition-all duration-300">
            <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
              <Globe size={14} className="text-blue-400" /> 区域状态
            </h3>
            <div className="space-y-2">
              {regionData.map((r, i) => (
                <motion.div
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    r.status === 'ok' ? 'bg-emerald-400 shadow-[0_0_4px_rgba(52,211,153,0.3)]' :
                    r.status === 'degraded' ? 'bg-amber-400 shadow-[0_0_4px_rgba(251,191,36,0.3)]' :
          ...
          <div className="bg-[#0d1117]/80 backdrop-blur-sm border border-white/[0.05] rounded-2xl p-5 hover:border-white/[0.08] transition-all duration-300">
            <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
              <Bell size={14} className="text-amber-400" /> 事件时间线
            </h3>
                ...
                >
                  <div className="relative mt-1">
                    <span className={`block w-1.5 h-1.5 rounded-full ${
                      e.level === 'critical' ? 'bg-red-400 shadow-[0_0_4px_rgba(248,113,113,0.4)]' :
                      e.level === 'warning' ? 'bg-amber-400 shadow-[0_0_4px_rgba(251,191,36,0.4)]' :
                      'bg-blue-400 shadow-[0_0_4px_rgba(96,165,250,0.4)]'
                    }`} />
                    {i < events.length - 1 && <div className="absolute top-3 left-0.5 w-px h-6 bg-white/[0.04]" />}
                  </div>
                  <div>
                    <p className="text-xs text-slate-300 leading-relaxed">{e.text}</p>
                    <p className="text-[10px] text-slate-600 mt-0.5">{e.time}</p>
                  </div>
                </motion.div>
              ))}
            </div>
